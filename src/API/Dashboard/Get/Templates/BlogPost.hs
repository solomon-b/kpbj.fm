...
renderBlogPostTableRow showModel post = do
  ...
  Lucid.tr_ [class_ $ base ["border-b", "border-gray-300", "hover:bg-gray-50"], Lucid.id_ postRowId] $ do
    -- Title
    ...
    -- Status
    Lucid.td_ [class_ $ base [Tokens.px4, "py-3", Tokens.textSm]] $
      renderStatusBadge post.status

    -- Actions
    Lucid.td_ [class_ $ base [Tokens.px4, "py-3", "text-right"]] $ do
      let editUrl = showBlogEditGetUrl showModel.id post.id post.slug
      Lucid.div_ [class_ $ base ["flex", Tokens.gap2, "justify-end"]] $
        Lucid.a_
