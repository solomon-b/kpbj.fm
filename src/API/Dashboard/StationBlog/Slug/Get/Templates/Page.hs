...
template post tags mAuthor = do
  Lucid.div_ [class_ $ base [Tokens.fullWidth]] $ do
    ...
    Lucid.div_ [class_ $ base [Tokens.bgMain, "rounded", Tokens.p6, Tokens.mb6]] $ do
      ...
      -- Metadata
      Lucid.div_ [class_ $ base ["grid", "grid-cols-2", Tokens.gap4, Tokens.textSm, Tokens.fgMuted, "mt-4", "pt-4", "border-t", Tokens.borderDefault]] $ do
        Lucid.div_ [] $ do
          Lucid.span_ [class_ $ base [Tokens.fontBold]] "Author: "
          case mAuthor of
            Just author -> Lucid.toHtml author.mDisplayName
            Nothing -> Lucid.span_ [class_ $ base [Tokens.fgMuted]] "Unknown"
        Lucid.div_ [] $ do
        ...
        Lucid.div_ [] $ do
          ...
          case post.bpmPublishedAt of
            ...
            Nothing -> Lucid.span_ [class_ $ base [Tokens.fgMuted]] "Not published"

    -- Tags
    if null tags
      then mempty
      else Lucid.div_ [class_ $ base [Tokens.bgMain, "rounded", Tokens.p6, Tokens.mb6]] $ do
    ...
    case post.bpmExcerpt of
      Just excerpt | not (Text.null excerpt) ->
        Lucid.div_ [class_ $ base [Tokens.bgMain, "rounded", Tokens.p6, Tokens.mb6]] $ do
          ...
          Lucid.p_ [class_ $ base [Tokens.fgMuted]] $
...
renderTag tag =
  Lucid.span_
    [class_ $ base ["px-3", "py-1", Tokens.textSm, Tokens.bgAlt, "rounded"]]
