...
renderShowHeader showModel hosts schedules = do
  ...
  case showModel.bannerUrl of
    Just bannerUrl -> do
      ...
      Lucid.div_ [Lucid.class_ $ cls [Tokens.fullWidth, Tokens.mb8, Tokens.cardBorder, "overflow-hidden"]] $ do
        Lucid.img_ [Lucid.src_ [i|/#{mediaGetUrl}/#{bannerUrl}|], Lucid.alt_ bannerAlt, Lucid.class_ $ cls [Tokens.fullWidth, "h-auto", "object-cover"]]
    Nothing -> mempty
  ...
  Lucid.section_ [Lucid.class_ $ cls [Tokens.bgWhite, Tokens.cardBorder, Tokens.p8, Tokens.mb8, Tokens.fullWidth]] $ do
    Lucid.div_ [Lucid.class_ $ cls ["grid", "grid-cols-1", lg "grid-cols-4", Tokens.gap8]] $ do
      -- Show Logo
      Lucid.div_ [Lucid.class_ $ cls [lg "col-span-1"]] $ do
        Lucid.div_ [Lucid.class_ $ cls [Tokens.fullWidth, "aspect-square", "bg-gray-300", Tokens.border2, "border-gray-600", "flex", "items-center", "justify-center", Tokens.textLg]] $ do
          case showModel.logoUrl of
            Just logoUrl -> do
              ...
              Lucid.img_ [Lucid.src_ [i|/#{mediaGetUrl}/#{logoUrl}|], Lucid.alt_ logoAlt, Lucid.class_ $ cls [Tokens.fullWidth, "h-full", "object-cover"]]
            Nothing -> "[SHOW IMAGE]"

      -- Show Info
      Lucid.div_ [Lucid.class_ $ cls [lg "col-span-3"]] $ do
        ...
        Lucid.div_ [Lucid.class_ Tokens.mb6] $ do
          Lucid.h2_ [Lucid.class_ $ cls [Tokens.textXl, Tokens.fontBold, "mb-3", "uppercase", "border-b", Tokens.borderGray800, Tokens.pb2]] "About The Show"
