  ...
  Lucid.div_ [Lucid.class_ $ cls [Tokens.bgWhite, Tokens.cardBorder]] $ do
    ...
    Lucid.div_ [Lucid.class_ $ cls ["border-b-2", "border-gray-800", Tokens.p6]] $ do
      Lucid.div_ [Lucid.class_ $ cls ["flex", Tokens.gap6, Tokens.mb6]] $ do
        -- Episode artwork
        Lucid.div_ [Lucid.class_ $ cls ["w-48", "h-48", "bg-gray-300", "border-2", "border-gray-600", "flex", "items-center", "justify-center", "text-xs", "flex-shrink-0"]] $ do
          case episode.artworkUrl of
            Just artworkUrl ->
              Lucid.img_
                [ Lucid.src_ [i|/#{mediaGetUrl}/#{artworkUrl}|],
                  Lucid.alt_ "Episode artwork",
                  Lucid.class_ $ cls [Tokens.fullWidth, "h-full", "object-cover"]
                ]
            Nothing -> "[EPISODE ARTWORK]"

        -- Episode metadata
        Lucid.div_ [Lucid.class_ "flex-grow"] $ do
          Lucid.div_ [Lucid.class_ $ cls ["flex", "items-start", "justify-between"]] $ do
            Lucid.div_ $ do
              Lucid.div_ [Lucid.class_ $ cls ["text-xs", "uppercase", "tracking-wide", Tokens.textGray600, Tokens.mb2]] $
                "Episode " <> Lucid.toHtml (show episode.episodeNumber)
              Lucid.h1_ [Lucid.class_ $ cls [Tokens.text2xl, Tokens.fontBold, Tokens.mb4]] $ Lucid.toHtml episode.title
            ...
            Lucid.a_
              [ Lucid.href_ [i|/#{editUrl}|],
                ...
                hxTarget_ "#main-content",
                hxPushUrl_ "true",
                Lucid.class_ $ cls [Tokens.bgGray800, Tokens.textWhite, Tokens.px4, "py-2", Tokens.textSm, Tokens.fontBold, "hover:bg-gray-700"]
              ]
              "Edit Episode"
          ...
          Lucid.div_ [Lucid.class_ $ cls ["grid", "grid-cols-2", Tokens.gap4, Tokens.textSm, Tokens.mb4]] $ do
            -- Status
            Lucid.div_ $ do
              Lucid.span_ [Lucid.class_ $ cls [Tokens.fontBold, Tokens.textGray700]] "Status: "
            ...
            Lucid.div_ $ do
              Lucid.span_ [Lucid.class_ $ cls [Tokens.fontBold, Tokens.textGray700]] "Scheduled: "
    ...
    case episode.audioFilePath of
      Just audioPath -> do
        ...
        Lucid.div_ [Lucid.class_ $ cls ["border-b-2", "border-gray-800", Tokens.p6]] $ do
          Lucid.h2_ [Lucid.class_ $ cls [Tokens.textLg, Tokens.fontBold, Tokens.mb4, "uppercase"]] "Preview Audio"

          Lucid.div_
            [ Lucid.class_ $ cls [Tokens.bgGray100, "border-2", "border-gray-600", Tokens.p6],
              xData_
                [i|{
                playerId: '#{playerId}',
                isPlaying: false,
    ...
    unless (null tracks) $ do
      Lucid.div_ [Lucid.class_ $ cls [Tokens.p6]] $ do
        Lucid.h2_ [Lucid.class_ $ cls [Tokens.textLg, Tokens.fontBold, Tokens.mb4, "uppercase", "border-b", "border-gray-800", "pb-2"]] $
...
renderTrackRow track = do
  Lucid.div_ [Lucid.class_ $ cls ["flex", "justify-between", "items-start", "p-3", "hover:bg-gray-50", "border-b", "border-gray-200"]] $ do
    -- Track number
...
errorTemplate errorMsg = do
  Lucid.div_ [Lucid.class_ $ cls [Tokens.bgWhite, Tokens.cardBorder, "p-8", "text-center"]] $ do
    Lucid.h1_ [Lucid.class_ $ cls [Tokens.text2xl, Tokens.fontBold, Tokens.mb4]] "Error Loading Episode"
    Lucid.p_ [Lucid.class_ $ cls [Tokens.textGray700, "mb-6"]] $ Lucid.toHtml errorMsg
...
notFoundTemplate showSlug = do
  ...
  Lucid.div_ [Lucid.class_ $ cls [Tokens.bgWhite, Tokens.cardBorder, "p-8", "text-center"]] $ do
    ...
    Lucid.p_ [Lucid.class_ $ cls [Tokens.textGray700, "mb-6"]] "We couldn't find the episode you're looking for."
    Lucid.a_
      [ Lucid.href_ [i|/#{backUrl}|],
        hxGet_ [i|/#{backUrl}|],
        hxTarget_ "#main-content",
