  ...
  Lucid.div_ [class_ $ base ["border", "border-gray-300", Tokens.p4], Lucid.id_ episodeCardId] $ do
    Lucid.div_ [class_ $ base ["flex", "justify-between", "items-start", Tokens.mb2]] $ do
      ...
      Lucid.div_ [class_ $ base ["flex", Tokens.gap2]] $ do
        Lucid.a_ [Lucid.href_ [i|/#{episodeEditUrl}|], hxGet_ [i|/#{episodeEditUrl}|], hxTarget_ "#main-content", hxPushUrl_ "true", class_ $ base ["bg-gray-600", Tokens.textWhite, Tokens.px3, "py-1", Tokens.textXs, Tokens.fontBold, "hover:bg-gray-700", "no-underline"]] "EDIT"
        Lucid.button_
          [ hxDelete_ [i|/#{episodeDelUrl}|],
            ...
            class_ $ base ["bg-red-600", Tokens.textWhite, Tokens.px3, "py-1", Tokens.textXs, Tokens.fontBold, "hover:bg-red-700"]
          ]
          "ARCHIVE"

    Lucid.p_ [class_ $ base [Tokens.textSm, Tokens.textGray700, "mb-3"]] $ do
    ...
    Lucid.div_ [class_ $ base ["flex", "justify-between", "items-center", Tokens.textXs, "text-gray-500"]] $ do
      Lucid.div_ $ do
        "Status: "
        Lucid.span_ [class_ $ base ["text-green-600", Tokens.fontBold]] "Published"
