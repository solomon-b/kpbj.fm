...
renderPostCard showModel post = do
  ...
  Lucid.article_
    [ class_ $ base [Tokens.bgWhite, Tokens.cardBorder, Tokens.p6, "hover:shadow-lg", "transition-shadow"],
      Lucid.id_ postId
    ]
    $ do
      -- Date
      ...
      -- Title
      Lucid.h2_ [class_ $ base [Tokens.text2xl, Tokens.fontBold, "mb-3"]] $ do
        Lucid.a_
      ...
      case ShowBlogPosts.excerpt post of
        Just exc -> do
          Lucid.p_ [class_ $ base [Tokens.textGray700, Tokens.mb4]] $ Lucid.toHtml exc
        Nothing -> do
          ...
          Lucid.p_ [class_ $ base [Tokens.textGray700, Tokens.mb4]] $ Lucid.toHtml excerpt

      -- Read more link and action buttons
      Lucid.div_ [class_ $ base ["flex", "items-center", "justify-between", "mt-4"]] $ do
        Lucid.a_
          [ Lucid.href_ [i|/#{showBlogPostGetUrl showModel post}|],
            hxGet_ [i|/#{showBlogPostGetUrl showModel post}|],
            hxTarget_ "#main-content",
