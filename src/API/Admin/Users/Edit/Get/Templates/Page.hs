...
module API.Admin.Users.Edit.Get.Templates.Page where

--------------------------------------------------------------------------------

import {-# SOURCE #-} API (adminUserDetailGetLink, adminUserEditPostLink)
    ...
    $ do
      ...
      Lucid.div_ [Lucid.class_ "flex gap-4 pt-4 border-t-2 border-gray-200"] $ do
        ...
        Lucid.a_
          ...
          "CANCEL"
  where
    backUrl = Links.linkURI $ adminUserDetailGetLink user.mId
    postUrl = Links.linkURI $ adminUserEditPostLink user.mId
