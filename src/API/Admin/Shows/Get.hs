...
module API.Admin.Shows.Get (Route, handler) where

--------------------------------------------------------------------------------

import {-# SOURCE #-} API (rootGetLink, userLoginGetLink)
import API.Admin.Shows.Get.Templates.Page (template)
    ...
    Just (_user, userMetadata) -> do
      getShowsResults limit offset queryFilter statusFilter >>= \case
        ...
        Right allShows -> do
          ...
          renderTemplate hxRequest (Just userMetadata) showsTemplate
