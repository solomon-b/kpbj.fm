...
renderWeekContent _year _weekNum _maybeTagFilter _eventTagsWithCounts weekGrid startDate endDate ((prevYear, prevWeek), (nextYear, nextWeek)) = do
  ...
  Lucid.section_ [Lucid.class_ $ cls [Tokens.bgWhite, Tokens.cardBorder, Tokens.p6]] $ do
    Lucid.div_ [Lucid.class_ $ cls ["flex", "items-center", "justify-between", Tokens.mb6]] $ do
      Lucid.h2_ [Lucid.class_ $ cls [Tokens.textXl, Tokens.fontBold]] $ Lucid.toHtml weekTitle
    ...
    -- Week Grid
    Lucid.div_ [Lucid.class_ $ cls ["grid", "grid-cols-1", md "grid-cols-7", Tokens.gap4]] $ do
      traverse_ renderWeekDay weekGrid

-- | Render a single day in the week view
renderWeekDay :: WeekDay -> Lucid.Html ()
renderWeekDay day = do
  Lucid.div_ [Lucid.class_ $ cls ["border", "border-gray-200", Tokens.p3]] $ do
    Lucid.div_ [Lucid.class_ $ cls [Tokens.fontBold, Tokens.mb2, "text-center"]] $
      Lucid.toHtml $
        wdDayName day <> " " <> Text.pack (show $ wdDayNumber day)

    if null (wdEvents day)
      then Lucid.div_ [Lucid.class_ $ cls [Tokens.textXs, "text-gray-400", "text-center", "mt-8"]] "No events"
...
renderWeekEvent event = do
  Lucid.div_ [Lucid.class_ $ cls ["bg-yellow-100", "text-yellow-800", Tokens.p2, Tokens.textXs]] $ do
