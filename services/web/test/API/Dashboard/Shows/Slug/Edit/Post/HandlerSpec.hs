...
-- | The same slot with the replay moved to 04:00. Nothing else differs.
replayMovedAgainJson :: Text
replayMovedAgainJson =
  "[{\"dayOfWeek\":\"monday\",\"weeksOfMonth\":[1,2,3,4,5],\"startTime\":\"20:00\",\"duration\":120,\"replayTime\":\"04:00\"}]"

-- | Cancelling a pending schedule reports the episodes it detached.
--
-- A second schedule edit cancels the pending slot from the first one. Episodes
-- booked against that slot lose their air time, and the save used to report none
-- of them, because the detach discarded its own result. Staff saw a plain success
-- and had no way to learn which episodes needed a new slot.
--
-- The detach itself is a separate question. This test pins the report only.
test_cancelPendingScheduleReportsDetachedEpisodes :: TestDBConfig -> IO ()
test_cancelPendingScheduleReportsDetachedEpisodes cfg = do
  userInsert <- mkUserInsert "edit-cancel-pending" UserMetadata.Staff
  now <- getCurrentTime
  let today = utctDay now
      changeDate = addDays 14 today
  ...
  let showInsert =
        Shows.Insert
          { Shows.siTitle = "Cancel Pending Show",
            Shows.siSlug = Slug "edit-cancel-pending-show",
            Shows.siDescription = Nothing,
            Shows.siLogoUrl = Nothing,
            Shows.siStatus = Shows.Active
          }
      startDateText = Just (Text.pack (show changeDate))
      firstForm =
        (editForm "Cancel Pending Show" "active")
          { sefSchedulesJson = Just replayMovedJson,
            sefScheduleStartDate = startDateText
          }
      secondForm =
        (editForm "Cancel Pending Show" "active")
          { sefSchedulesJson = Just replayMovedAgainJson,
            sefScheduleStartDate = startDateText
          }
  ...
  bracketAppM cfg $ do
    dbResult <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
      ...
      showModel <-
        TRX.statement () (Shows.getShowById showId)
          >>= maybe (error "show not found") pure
      pure (userMetaModel, showModel, episodeId)

    (userMetaModel, showModel, episodeId) <- liftIO $ expectSetupRight dbResult

    -- Save 1 defers the replay change, so the episode moves onto a pending slot.
    -- It also regenerates the slug from the title, so save 2 uses the new one.
    firstResult <- runExceptT $ action userMetaModel showModel.slug firstForm
    (slugAfterFirst, firstUnscheduled) <- case firstResult of
      Left err -> error $ "Expected the first edit to succeed but got Left: " <> show err
      Right (_, slug, unscheduled) -> pure (slug, unscheduled)
    liftIO $ firstUnscheduled `shouldBe` []

    -- Save 2 retimes the replay again, which cancels the pending slot.
    secondResult <- runExceptT $ action userMetaModel slugAfterFirst secondForm
