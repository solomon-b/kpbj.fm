  ...
  Lucid.div_ [class_ $ base [Tokens.bgMain, "rounded", "border", Theme.borderMuted]] $ do
      ...
      Lucid.div_ [class_ $ base ["flex", Tokens.gap6, Tokens.mb6]] $ do
        ...
        Lucid.div_ [Lucid.class_ "flex-grow"] $ do
          Lucid.div_ $ do
            ...
            Lucid.div_ [class_ $ base ["text-xs", "uppercase", "tracking-wide", Tokens.fgMuted, Tokens.mb2]] $
          ...
          Lucid.div_ [class_ $ base ["grid", "grid-cols-2", Tokens.gap4, Tokens.textSm, Tokens.mb4]] $ do
            ...
            case episode.durationSeconds of
              Just duration -> do
                ...
                Lucid.div_ $ do
                  Lucid.span_ [class_ $ base [Tokens.fontBold, Tokens.fgMuted]] "Duration: "
                  if hours > 0
          ...
          case episode.description of
            Just desc -> Lucid.p_ [class_ $ base [Tokens.fgMuted, "leading-relaxed"]] $ Lucid.toHtml desc
            Nothing -> mempty

          -- Episode tags
          unless (null tags) $ do
            Lucid.div_ [class_ $ base [Tokens.mt4]] $ do
              Lucid.span_ [class_ $ base [Tokens.fontBold, Tokens.fgMuted, "mr-2"]] "Tags:"
    ...
    unless (null tracks) $ do
      Lucid.div_ [class_ $ base [Tokens.p6]] $ do
        Lucid.h2_ [class_ $ base [Tokens.textLg, Tokens.fontBold, Tokens.mb4, "uppercase", "border-b", Tokens.borderDefault, "pb-2"]] $
...
renderTag tag =
  Lucid.span_
    [class_ $ base [Tokens.bgAlt, Tokens.fgMuted, "px-2", "py-1", "text-xs", "font-mono", "rounded"]]
    $ Lucid.toHtml
...
renderTrackRow track = do
  Lucid.div_ [class_ $ base ["flex", "justify-between", "items-start", "p-3", Tokens.hoverBg, "border-b", Tokens.borderDefault]] $ do
    -- Track number
    ...
    -- Track info
    Lucid.div_ [Lucid.class_ "flex-grow"] $ do
      Lucid.div_ [class_ $ base ["font-medium", Tokens.fgPrimary]] $ do
