...
data ScheduleUpdate = ScheduleUpdate
  { -- | Upcoming episodes the change detached. Named in the flash message.
    --
    -- Covers both detach paths: the cancelled pending templates and the active
    -- templates the diff removed.
...
--
-- Returns the pending templates it cancelled, and the episodes it detached.
cancelPendingSchedule ::
  [ShowSchedule.ScheduleTemplate Result] ->
  [ShowSchedule.ScheduleTemplate Result] ->
  HT.Transaction ([ShowSchedule.TemplateId], [Episodes.UpcomingEpisodeRef])
cancelPendingSchedule pendingTemplates activeTemplates = do
  -- Cancel pending validity periods
  detached <- forM pendingTemplates $ \template -> do
    validities <- HT.statement () (ShowSchedule.getValidityPeriodsForTemplate template.stId)
    ...
    -- one covers them.
    --
    -- Read them first, and from the same date, so the caller can name them in the
    -- flash. Both statements apply the same three gates, so the list names every
    -- episode the clear takes and no other.
    case map (.stvEffectiveFrom) validities of
      [] -> pure []
      froms -> do
        let fromDate = minimum froms
        episodes <- HT.statement () (Episodes.getUpcomingEpisodesForTemplates [template.stId] fromDate)
        void $ HT.statement () (Episodes.clearTemplateForUpcomingEpisodes template.stId fromDate)
        pure episodes
