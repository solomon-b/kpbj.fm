  ...
  Lucid.script_ $
    Text.unlines
      [ "let trackCount = 0;",
        ...
        "  const audioFileInput = document.getElementById('main-file');",
        ...
        "      console.log('File selected, change event fired');",
        "      const file = e.target.files[0];",
        "      console.log('File:', file);",
        "      console.log('File type:', file ? file.type : 'no file');",
        "      console.log('File name:', file ? file.name : 'no file');",
        "      ",
        "      // Check if file is audio by MIME type or extension",
        "      const isAudio = file && (file.type.startsWith('audio/') || /\\.(mp3|wav|flac|aac|ogg|m4a)$/i.test(file.name));",
        "      console.log('Is audio file:', isAudio);",
        "      ",
        "      if (file && isAudio) {",
        "        console.log('Processing audio file:', file.type || 'unknown type');",
        "        const audio = new Audio();",
        "        audio.preload = 'metadata';",
        "        ",
        "        audio.onloadedmetadata = function() {",
        "          const durationSeconds = Math.round(audio.duration);",
        "          console.log('Audio duration detected:', durationSeconds, 'seconds');",
        "          console.log('Duration field before setting:', durationField);",
        "          ",
        "          if (durationField) {",
        "            durationField.value = durationSeconds;",
        "            console.log('Duration field value set to:', durationField.value);",
        "          } else {",
        ...
        "      } else {",
        "        console.log('File is not audio type or no file selected');",
        "      }",
        "    });",
        "  } else {",
        ...
        "  }",
        "  ",
        "  // Log form submission to verify duration field value",
        "  const form = document.querySelector('form');",
        "  if (form) {",
        "    form.addEventListener('submit', function(e) {",
        "      const durationValue = document.getElementById('duration-seconds')?.value;",
        "      console.log('Form submitting with duration value:', durationValue);",
        "    });",
        "  }",
        "});"
      ]
