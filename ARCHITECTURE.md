...
**VPS Configuration**: Streaming droplets run NixOS with Podman containers managed by systemd. Configuration is declarative in `nixos/`:

```
nixos/
  common.nix              # SSH, firewall, Podman, base packages
  ...
  streaming.nix           # OCI containers (Icecast, Liquidsoap, Webhook)
  nginx.nix               # Reverse proxy + ACME (Let's Encrypt)
  sops.nix                # sops-nix secrets (decrypts streaming env file)
...
3. sops-nix renders a `kpbj-stream.env` template and restarts affected containers
