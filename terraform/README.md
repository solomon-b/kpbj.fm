...
Sensitive values (API tokens, zone IDs) are stored in `secrets.yaml`, encrypted with SOPS + age.
...
- **Docker Compose on VPS** — deployed via `just stream-*-deploy`
- **Nginx + certbot on VPS** — one-time manual setup
- **fly.toml / fly.staging.toml** — still used by CI deploy workflows
...
| `secrets.yaml`             | SOPS-encrypted secret values                                  |
