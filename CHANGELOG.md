...
### Fixed
- **A show no longer over-runs or gets cut by a daylight-saving change** — `getCurrentlyAiringEpisode` compared the current `time` against the template's `start_time` and `end_time`. Subtracting two `time` values gives a different answer from subtracting the matching `timestamptz` values on the two transition dates each year, so the comparison was wrong on both. On 2025-11-02 the Pacific clock runs 01:00 to 02:00 twice, so a `00:00` to `02:00` slot covers 3 elapsed hours and the `time` `01:30` matches both 08:30 UTC and 09:30 UTC. A 2-hour episode therefore stayed "on air" for about an hour after its audio ended, and Liquidsoap re-served the same file, which gave dead air or a stall. The query now builds each airing window as a pair of `timestamptz` values and asks whether the current time falls between them. The episode stops at the end of its slot or at the end of its audio, whichever comes first. This also removed the six-case structure the `time` arithmetic needed. Cases 2, 3, 5 and 6 existed only to split an overnight window at midnight, and there is no midnight to split when the comparison uses `timestamptz`, so about 90 lines of `CASE` became one comparison over a primary window and a replay window. A slot shortened by the spring-forward change still cuts the episode at the slot's end, because the next show starts then.
- **The stream no longer picks arbitrarily between two shows that claim the same time** — The airing query ended in `LIMIT 1` with no `ORDER BY`, so PostgreSQL returned whichever row the plan reached first. Two overlapping slots could therefore swap on the stream between one poll and the next. The query now orders a primary airing ahead of a replay, then by air time and id, so the choice is stable. A new `getCurrentlyAiringEpisodes` returns up to two rows, and `/api/playout/now` logs an attention entry naming the episodes when a second one comes back. The overlap itself is still a data defect, and the conflict checks remain the thing that prevents it.
