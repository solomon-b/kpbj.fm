    ...
    getAuth cfg =
      bracketConn cfg $ do
        ...
        auth <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
          ...
          _ <- TRX.statement () $ UserMetadata.insertUserMetadata $ UserMetadata.Insert (getOneRow uid) (mkDisplayNameUnsafe "Test User") (mkFullNameUnsafe "Test User") Nothing UserMetadata.Host UserMetadata.Automatic
...
withTestDB = around $ bracket before after . during
  where
    ...
    before = do
      ...
      uuid <- nextRandom
      let dbname = "kpbj-test-db-" <> toString uuid
      (ec, _, stde) <-
        readProcess $
          proc
            "psql"
