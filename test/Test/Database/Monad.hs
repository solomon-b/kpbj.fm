    ...
    getAuth cfg =
      bracketConn cfg $ do
        ...
        auth <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
          ...
          _ <- TRX.statement () $ UserMetadata.insertUserMetadata $ UserMetadata.ModelInsert (getOneRow uid) (mkDisplayNameUnsafe "Test User") (mkFullNameUnsafe "Test User") Nothing UserMetadata.Host
