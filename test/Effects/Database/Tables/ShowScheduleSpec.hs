  ...
  arrange (bracketConn cfg) $ do
    ...
    act $ do
      ...
      result <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
        ...
        let validityInsert = UUT.ValidityInsert templateId (addDays (-7) today) Nothing
        void $ TRX.statement () (UUT.insertValidity validityInsert)
        upcomingDates <- TRX.statement () (UUT.getUpcomingShowDates showId 10)
        pure (today, upcomingDates)

      assert $ do
        (_today, upcomingDates) <- assertRight result
  ...
  arrange (bracketConn cfg) $ do
    ...
    act $ do
      ...
      result <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
        ...
        TRX.statement () (UUT.getUpcomingShowDates showId 4)
  ...
  arrange (bracketConn cfg) $ do
    ...
    act $ do
      ...
      result <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
        ...
        let validityInsert = UUT.ValidityInsert templateId (addDays (-7) today) Nothing
        void $ TRX.statement () (UUT.insertValidity validityInsert)
        upcomingDates <- TRX.statement () (UUT.getUpcomingShowDates showId 5)
  ...
  arrange (bracketConn cfg) $ do
    ...
    act $ do
      ...
      result <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
        ...
        upcomingDates <- TRX.statement () (UUT.getUpcomingShowDates showId 20)
  ...
  arrange (bracketConn cfg) $ do
    ...
    act $ do
      ...
      result <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
        ...
        -- Set validity to start late in the current year (or next year if we're in December)
        let startYear = if currentMonth == 12 then currentYear + 1 else currentYear
            effectiveFrom = fromGregorian startYear 12 15 -- Mid-December
            validityInsert = UUT.ValidityInsert templateId effectiveFrom Nothing
        void $ TRX.statement () (UUT.insertValidity validityInsert)

        TRX.statement () (UUT.getUpcomingShowDates showId 10)

      assert $ do
        upcomingDates <- assertRight result
        -- Should get dates spanning year boundary
        case upcomingDates of
          (d1 : rest) | not (null rest) -> do
            ...
            -- Check if we cross year boundary
            let crossesBoundary = any (> year1) years
            Hedgehog.assert crossesBoundary
          _ -> pure () -- Not enough dates or edge case
  ...
  arrange (bracketConn cfg) $ do
    ...
    act $ do
      ...
      result <- runDB $ TRX.transaction TRX.ReadCommitted TRX.Write $ do
        ...
        allDates <- TRX.statement () (UUT.getUpcomingShowDates showId 10)
